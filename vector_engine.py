...
# vector_engine.py - Text embedding and similarity computation engine
...
def embed(text: str, debug: bool = False) -> np.ndarray:
    """
    Generate vector embedding for input text using the loaded model.
...
def embed_with_context(text: str, persona: str, job_to_be_done: str, debug: bool = False) -> np.ndarray:
    """
    Generate contextual embedding by combining text with persona and job context.
    ...
    Returns:
        numpy array containing the contextual embedding
    """
    try:
